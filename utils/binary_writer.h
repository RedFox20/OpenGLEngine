...
struct write_buffer
{
	...
	inline write_buffer& write(const token& str)
	{
		return write(str.c_str(), str.length());
	}
	...
	inline write_buffer& writeln(const token& str)
	{
		return write(str.c_str(), str.length()).write('\n');
	}
	...
	write_buffer& write_padded(int minWidth, const token& str)
	{
...
struct binary_buffer : public write_buffer
{
	...
	binary_buffer& write_cstr(const token& str)
	{
		write(str.str, int(str.end - str.str)).write('\0'); return *this;
	}
	/** @brief Write a length specified string to the buffer in the form of [len][data] */
	binary_buffer& write_nstr(const token& str)
	{
		int len = int(str.end - str.str);
		write_int(len).write(str.str, len); return *this;
	}
	/** @brief Write a bastardized length specified and null-terminated string to the buffer in the form of [len][data][0] */
	binary_buffer& write_ncstr(const token& str)
	{
		int len = int(str.end - str.str);
		write_int(len).write(str.str, len).write('\0'); return *this;
	}
...
struct string_buffer : public write_buffer
{
	...
	inline string_buffer& write(const write_buffer& other)
	{
		write_buffer::write(other); return *this;
	}
	/** @brief Writes a regular string into the buffer */
	inline string_buffer& write(const std::string& str)
	{
		write_buffer::write(str); return *this;
	}
	/** @brief Writes a wide string into the buffer */
	inline string_buffer& write(const std::wstring& str)
	{
		write_buffer::write(str); return *this;
	}
	/** @brief Writes a token string into the buffer */
	inline string_buffer& write(const token& str)
	{
		write_buffer::write(str); return *this;
	}
	/** @brief Writes a regular string literal into the buffer */
	template<int SIZE> inline string_buffer& write(const char (&str)[SIZE])
	{
		write_buffer::write(str, SIZE - 1); return *this;
	}
	/** @brief Writes a wide string literal into the buffer */
	template<int SIZE> inline string_buffer& write(const wchar_t (&str)[SIZE])
	{
	...
	string_buffer& write_padded(int minWidth, const token& str)
	{
...
inline write_buffer& _strmwritebuf(write_buffer& os, const load_buffer& lb)
{
	return os.write(lb.Buffer, lb.Size);
}
...
inline write_buffer& operator<<(write_buffer& os, const token& s)
{
	return os.write(s.str, int(s.end - s.str));
}
template<int SIZE> inline write_buffer& operator<<(write_buffer& os, char (&str)[SIZE])
{
	return os.write(str, strlen(str));
}
template<int SIZE> inline write_buffer& operator<<(write_buffer& os, wchar_t (&str)[SIZE])
{
	return os.write(str, wcslen(str) * sizeof(wchar_t));
}
template<int SIZE> inline write_buffer& operator<<(write_buffer& os, const char (&str)[SIZE])
{
	return os.write(str, SIZE - 1);
}
template<int SIZE> inline write_buffer& operator<<(write_buffer& os, const wchar_t (&str)[SIZE])
{
...
inline string_buffer& _strmwritebuf(string_buffer& os, const load_buffer& lb)
{
	return os.write(lb.Buffer, lb.Size);
}
...
inline string_buffer& operator<<(string_buffer& os, const token& s)
{
	return os.write(s.str, int(s.end - s.str));
}
template<int SIZE> inline string_buffer& operator<<(string_buffer& os, char (&str)[SIZE])
{
	return os.write(str, strlen(str));
}
template<int SIZE> inline string_buffer& operator<<(string_buffer& os, wchar_t (&str)[SIZE])
{
	return os.write(str, wcslen(str) * sizeof(wchar_t));
}
template<int SIZE> inline string_buffer& operator<<(string_buffer& os, const char (&str)[SIZE])
{
	return os.write(str, SIZE - 1);
}
template<int SIZE> inline string_buffer& operator<<(string_buffer& os, const wchar_t (&str)[SIZE])
{
	return os.write(str, sizeof(str)-sizeof(wchar_t));
}
...
{
	unbuffered_file File;			// the destination file
	...
	inline basic_filewriter(unbuffered_file&& file)								: File(file) {}
	/** @brief Creates a new filewriter object by creating/opening a file */
	inline basic_filewriter(const char* filename, IOFlags mode = CREATENEW)		: File(filename, mode) {}
	/** @brief Creates a new filewriter object by creating/opening a file */
	inline basic_filewriter(const std::string& filename, IOFlags mode = CREATENEW) : File(filename, mode) {}
	inline ~basic_filewriter()
	{
	 ...
	 */
	inline bool open(const char* filename, IOFlags mode = CREATENEW)
	{
	 ...
	 * @return TRUE if file was opened successfully, FALSE if failed
	 */
	inline bool open(const std::string& filename, IOFlags mode = CREATENEW)
	{
		return File.open(filename, mode);
	}
...
template<class WriteBufferType> struct basic_streamwriter : public WriteBufferType
{
	...
	inline basic_streamwriter() {}
	/** @brief Initialize streamwriter from an already opened file, taking ownership of the file object */
	inline basic_streamwriter(file&& file)											  : Stream(file) {}
	/** @brief Initialize streamwriter from the specified filename */
	inline basic_streamwriter(const char* filename, IOFlags mode = CREATENEW)		  : Stream(filename, mode) {}
	/** @brief Initialize streamwriter from the specified filename */
	inline basic_streamwriter(const std::string& filename, IOFlags mode = CREATENEW) : Stream(filename, mode) {}
	inline ~basic_streamwriter()
	{
		flush(); // flush on destruction
	}
	...
	inline int seek(int seekpos, int seekmode = SEEK_SET)
	{
